...
#
# Z Calibration RC backend is deliberately observational. Z-Mod owns physical
# Auto-Z/contact and standard Klipper owns the effective gcode_offset. This
# component explains the resulting state; it does not write Z or own motion.
...
def _derive_zmod_provenance(
    ...
    """Derive only provenance supported by the accepted Z-Mod saved+check path.

    Proven live-source semantics at Z-Mod 1.7.2-5 / 2e32155d...:
    - gcode_offsets.z is the persisted/global Z state with temp_z_offset removed;
    - _TEST_POINT.temp_z_offset is Z-Mod's temporary saved-mesh alignment;
    - when screen=False and load_zoffset=1, START_PRINT ignores slicer Z_OFFSET.

    Arbitrary residual is never guessed to be babystepping. It remains
    external_unknown until a future explicit Plugins AD5X action can attribute it.
    """
    if zcore is None:
...
class PluginsAD5X:
    ...
    def _handle_job_state_changed(self, *args: Any) -> None:
        # No offset attribution is guessed from a job event. The event only
        # invalidates cached UI state; the next explicit snapshot re-reads
        # standard Klipper/Z-Mod objects.
        state = None
        for item in args:
    ...
    def invalidate_snapshot(self) -> int:
        ...
        if callable(send_event):
            send_event(SNAPSHOT_CHANGED_EVENT, {"revision": self._revision})
