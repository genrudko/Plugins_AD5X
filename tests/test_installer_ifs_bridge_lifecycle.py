...
class IFSBridgeInstallerLifecycleTests(unittest.TestCase):
    def run_shell(self, body: str, tmp: Path, *, check: bool = True) -> subprocess.CompletedProcess[str]:
        ...
        moon = tmp / "moonraker"
        (moon / "components").mkdir(parents=True, exist_ok=True)
        ...
        env.update(
            {
                ...
                "AD5X_BACKEND_DEST": str(moon / "components" / "plugins_ad5x.py"),
                "AD5X_MOONRAKER_COMPONENTS_DIR": str(moon / "components"),
    ...
    def test_default_destination_matches_live_ad5x_klipper_runtime(self) -> None:
        text = INSTALLER.read_text(encoding="utf-8")
        self.assertIn('KLIPPER_BRIDGE_DEST="${AD5X_KLIPPER_BRIDGE_DEST:-/opt/config/base/klipper/klippy/extras/ad5x_ifs.py}"', text)
        self.assertNotIn('KLIPPER_BRIDGE_DEST="${AD5X_KLIPPER_BRIDGE_DEST:-/usr/prog/klipper/klippy/extras/ad5x_ifs.py}"', text)

    def test_deploy_creates_plugin_owned_symlink_and_git_exclude(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            result = self.run_shell("deploy_klipper_bridge_plugin_link", tmp, check=False)
            self.assertEqual(result.returncode, 0, result.stderr)
            dest = tmp / "klipper" / "klippy" / "extras" / "ad5x_ifs.py"
            self.assertTrue(dest.is_symlink())
            self.assertEqual(os.readlink(dest), str(BRIDGE))
            exclude = (tmp / "klipper" / ".git" / "info" / "exclude").read_text().splitlines()
            self.assertIn("/klippy/extras/ad5x_ifs.py", exclude)
            self.assertFalse((tmp / "state-root" / "state" / "klipper-ifs-bridge.sha256").exists())

    def test_existing_legacy_managed_copy_is_migrated(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            dest = tmp / "klipper" / "klippy" / "extras" / "ad5x_ifs.py"
    ...
    def test_foreign_destination_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            result = self.run_shell("validate_klipper_bridge_destination_ownership", tmp, check=False)
            self.assertNotEqual(result.returncode, 0)
            self.assertEqual(dest.read_text(), "foreign\n")

    def test_uninstall_removes_only_owned_link_bytecode_and_exclude(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            first = self.run_shell("deploy_klipper_bridge_plugin_link", tmp, check=False)
            self.assertEqual(first.returncode, 0, first.stderr)
            extras = tmp / "klipper" / "klippy" / "extras"
            pycache = extras / "__pycache__"
            pycache.mkdir(exist_ok=True)
            (pycache / "ad5x_ifs.cpython-312.pyc").write_bytes(b"x")
            result = self.run_shell("remove_klipper_bridge_plugin_link", tmp, check=False)
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertFalse((extras / "ad5x_ifs.py").exists())
            self.assertFalse(any(pycache.glob("ad5x_ifs*.pyc")))
            exclude = (tmp / "klipper" / ".git" / "info" / "exclude").read_text()
            self.assertNotIn("ad5x_ifs.py", exclude)

    def test_config_activation_exists_and_link_precedes_backend_transition(self) -> None:
        ...
        self.assertLess(install.index("deploy_klipper_bridge_plugin_link"), install.index("run_moonraker_transition backend_install_transition"))
    ...
    def test_installer_does_not_claim_firmware_restart_reloads_python_extra(self) -> None:
        ...
        self.assertIn("firmware restart недостаточен", text)

    def test_update_hook_relinks_bridge_without_service_transition(self) -> None:
        text = INSTALLER.read_text(encoding="utf-8")
        ...
        self.assertIn('runtime-restart-required', body)
