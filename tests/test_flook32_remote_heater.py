...
class FakePrinter:
    def __init__(self): self.gcode = FakeGcode(); self.heaters = FakeHeaters()
...
class FakeConfig:
    ...
    def get_name(self): return 'flook32 chamber'
...
def make_heater(values=None):
    ...
    return printer, sensor, flook32.FLOOK32RemoteHeater(FakeConfig(printer, values), sensor)
...
class RemoteHeaterTests(unittest.TestCase):
    ...
    def test_native_loader_registration(self):
        printer = FakePrinter(); config = FakeConfig(printer); sensor = FakeSensor()
        def fake_sensor(config_arg, register_temperature_object=True):
            self.assertFalse(register_temperature_object); return sensor
        with mock.patch.object(flook32, 'FLOOK32Sensor', fake_sensor):
            heater = flook32.load_config_prefix(config)
        self.assertIs(printer.heaters.heaters['chamber'], heater)
        self.assertEqual(printer.heaters.available_heaters, ['flook32 chamber'])
        self.assertEqual(printer.heaters.registered_sensors, [('flook32 chamber', heater)])
