...
sed "s#@TOUCH_EVENT@#$TOUCH_EVENT#g" /opt/ad5x-x11/xorg.conf.in > "$CONF"
...
i=0
while [ "$i" -lt 10 ]; do
    if DISPLAY=:0 xinput --list >/tmp/ad5x-xinput.txt 2>&1; then
        READY=1
        break
    fi
...
DISPLAY=:0 xsetroot -solid '#203050'
...
DISPLAY=:0 xev -geometry 640x360+80+60
