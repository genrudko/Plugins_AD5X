...
if [ ! -f "$TARGET/flook32.cfg" ]; then
    cp -p "$SRC_CFG" "$TARGET/flook32.cfg"
    echo 'CONFIG: installed default flook32.cfg'
else
    echo 'CONFIG: existing flook32.cfg preserved'
fi
...
MIGRATE_RESULT="$("$RUNTIME_PY" "$MIGRATE_CFG" "$TARGET/flook32.cfg")"
