...
class FLOOK32Sensor:
    ...
    def __init__(self, config):
        ...
        # Existing FLOOK32 configs keep this object unchanged.
        ...
        # Native Fluidd/Klipper heater proxy.  The existing temperature sensor
    ...
    def set_target_temperature(self, temp):
        """Set FLOOK32 target and keep Klipper-side state in sync."""
        temp = float(temp)
        if temp < 0.0 or temp > 70.0:
            return False, "FLOOK32 target must be between 0 and 70C"
        if not self.flook_ip:
            return False, "FLOOK32 is not connected"
        ...
        if result and ("OK" in result or "success" in result):
            ...
            return True, None
        return False, "FLOOK32 rejected target {:.1f}C".format(temp)

    def cmd_FLOOK_SET(self, gcmd):
        """FLOOK_SET S=<temperature> - set target temperature."""
        temp = gcmd.get_float('S', 0.0)
        ok, error = self.set_target_temperature(temp)
        if ok:
            gcmd.respond_info("FLOOK32 target set to {}C".format(temp))
        else:
...
class FLOOK32RemoteHeater:
    """Expose FLOOK32 as a native Klipper heater without owning the SSR.

    FLOOK32/ESP32 remains the physical controller and owns all safety logic.
    Klipper only exposes current/target/power state and forwards target changes.
    """
    ...
    def __init__(self, config, sensor, heater_name):
        ...
        self.full_name = "heater_generic " + heater_name
        self.min_temp = 0.0
        self.max_temp = sensor.native_heater_max_temp
        self.wait_delta = sensor.native_heater_wait_delta
    ...
    def set_temp(self, degrees):
        ...
        if degrees and (degrees < self.min_temp or degrees > self.max_temp):
            raise self.printer.command_error(
                'Requested FLOOK32 temperature (%.1f) out of range (%.1f:%.1f)'
                % (degrees, self.min_temp, self.max_temp))
        ok, error = self.sensor.set_target_temperature(degrees)
        if not ok:
            if degrees <= 0.0:
                # TURN_OFF_HEATERS must never be blocked by a disappeared
                # network accessory.  FLOOK32 retains its own autonomous
                # safety controller; keep Klipper target state safe and warn.
                with self.sensor.temp_lock:
                    self.sensor.target_temp = 0.0
                    self.sensor.heater_state = False
                logging.warning("FLOOK32 remote shutdown could not be confirmed: %s",
                                error)
                return
    ...
    def get_temp(self, eventtime):
        with self.sensor.temp_lock:
            return self.sensor.air_temp, self.sensor.target_temp

    def check_busy(self, eventtime):
        with self.sensor.temp_lock:
            target = self.sensor.target_temp
            current = self.sensor.air_temp
        return target > 0.0 and current < target - self.wait_delta

    def stats(self, eventtime):
        with self.sensor.temp_lock:
            temp = self.sensor.air_temp
            target = self.sensor.target_temp
            heater_on = self.sensor.heater_state
        active = bool(target > 0.0 or heater_on)
        return active, '%s: target=%.1f temp=%.1f remote=%s' % (
            self.name, target, temp, 'on' if heater_on else 'off')

    def get_status(self, eventtime):
        with self.sensor.temp_lock:
            temp = self.sensor.air_temp
            target = self.sensor.target_temp
            heater_temp = self.sensor.heater_temp
            heater_on = self.sensor.heater_state
            fan_on = self.sensor.fan_state
            fan_duty = self.sensor.fan_duty
            locked = self.sensor.system_locked
        return {
            'temperature': round(temp, 2),
            'target': target,
            'power': 1.0 if heater_on else 0.0,
            'heater_temperature': round(heater_temp, 2),
            'fan': fan_on,
            'fan_duty': fan_duty,
            'connected': bool(self.sensor.flook_ip),
            'websocket_connected': bool(self.sensor.ws_connected),
            'locked': locked,
            'flook_ip': self.sensor.flook_ip or '',
        }

    cmd_SET_HEATER_TEMPERATURE_help = 'Set FLOOK32 chamber temperature'
...
def _register_remote_heater(config, sensor):
    if not sensor.native_heater_enabled:
        return None

    printer = config.get_printer()
    pheaters = printer.load_object(config, 'heaters')
    heater_name = sensor.native_heater_name
    full_name = 'heater_generic ' + heater_name

    if heater_name in pheaters.heaters:
        raise config.error("Heater %s already registered" % (heater_name,))

    heater = FLOOK32RemoteHeater(config, sensor, heater_name)
