...
class FLOOK32Sensor:
    ...
    def __init__(self, config, register_temperature_object=True):
        """
        ...
        if register_temperature_object:
...
class FLOOK32RemoteHeater:
    """Expose FLOOK32 through Klipper's standard heaters API.

    The ESP32 remains the physical heater controller. Klipper only exposes
    current/target/power state and forwards target changes to FLOOK32.
    """
    ...
    def __init__(self, config, sensor):
        ...
        self.max_temp = config.getfloat('max_temp', 70.0,
                                        above=self.min_temp, maxval=70.0)
        self.wait_delta = config.getfloat('wait_delta', 2.0, minval=0.0)
    ...
    def set_temp(self, degrees):
        ...
        if ok:
            return
        if degrees <= 0.0:
            # Never let an unreachable remote chamber heater prevent Klipper
            # from turning off the remaining local heaters.  FLOOK32 owns its
            # physical fail-safes; keep the local UI target at zero and warn.
...
def _register_remote_heater(config, sensor):
    ...
    heater = FLOOK32RemoteHeater(config, sensor)
...
def load_config_prefix(config):
    """Native remote-heater form: [flook32 chamber]."""
    sensor = FLOOK32Sensor(config, register_temperature_object=False)
    return _register_remote_heater(config, sensor)
